...
##         TypeError: Cannot convert sage.matrix.matrix_integer_sparse.Matrix_integer_sparse to sage.matrix.matrix_integer_dense.Matrix_integer_dense
        ...
        AbelianGroup_class.__init__(self, self.__elementary_divisors, names)
    ...
    def _latex_(self):
        ...
        EXAMPLES:
            ...
            0 & 0 & 0 & 27 & 0 \\
            0 & 0 & 0 & 0 & 27
            \end{array}\right)\Z^{5} \right\rangle, \frac{1}{3} x_{0}^{2} + \frac{1}{3} x_{1}^{2} + \frac{1}{27} x_{2}^{2} + \frac{1}{27} x_{3}^{2} + \frac{1}{27} x_{4}^{2}\right)
        """
        n = self.ngens()
...
##             sage: B = A + A; B
    ...
    def __mul__(self, _n):
        ...
        if n < 0:
            raise ValueError("Argument n (= {0}) must be nonnegative.".format(n))
        if 0 == n:
...
##             2
    ...
    def subgroups(self, d=None):
        ...
        for h in __enumerate_echelon_forms():
            ...
            if FiniteQuadraticModule_subgroup._divides(h1, self.__E):
                f = FiniteQuadraticModule_subgroup(tuple([FiniteQuadraticModuleElement(self, list(x),
                                                                                 can_coords=False)
    ...
    def _is_valid_homomorphism_(self, codomain, im_gens):
        ...
        if not x.universe() is codomain:
            raise TypeError('im_gens (={0}) must belong to {1}'.format(im_gens, codomain))
    ...
    def _Hom_(self, B, cat = None):
        ...
        EXAMPLES::
            ...
            Set of Morphisms from Finite quadratic module in 2 generators:
                 ...
                 gens: e1, e1, e3, e3
                 form: 1/3*x0^2 + 2/3*x0*x1 + 1/3*x1^2 + 1/3*x2^2 + 2/3*x2*x3 + 1/3*x3^2 in Category of commutative additive groups
    """
...
class FiniteQuadraticModuleHomset (HomsetWithBase):
    ...
    def __call__(self, im_gens, check=True):
        ...
        try:
            ...
            print(err)
            raise TypeError("images (={0}) do not define a valid homomorphism".format(im_gens))
...
def _A(q, s = 1, **args):
    ...
    raise ValueError('q (={0}) must be prime power and s (={1}) relatively prime to q'.format(q,s))
...
def _FiniteQuadraticModule_from_string(S, **args ):
    ...
    EXAMPLES::
        ...
        Finite quadratic module in 4 generators:
         gens: a, b, c, d
         form: 1/3*x0^2 + 2/3*x1^2 + 1/5*x2^2 + 1/5*x3^2
...
def FiniteQuadraticModule(arg0=None, arg1=None, **args):
    ...
    EXAMPLES::
        ...
        Finite quadratic module in 3 generators:
         gens: x, y, z
         form: 6/7*x0^2 + 2/3*x1^2 + 3/4*x2^2
...
class FiniteQuadraticModule_subgroup(AbelianGroup_class):
    r"""
...
##         (30, 4, 120)
    ...
    def __init__(self, gens):
        ...
        try:
            ...
            raise TypeError("{0}: must be a list of elements of a quadratic module.".format(gens[0]))
        ...
        # names = tuple(x._repr_() for x in self.__hnf_gens)
        orders = tuple(ZZ(x.order()) for x in self.__hnf_gens)
        names = "f"
        AbelianGroup_class.__init__(self, orders, names)
    ...
    def __add__(self, other):
        ...
        EXAMPLES NONE
        """
        return FiniteQuadraticModule_subgroup(tuple(self.gens() + other.gens()))
    ...
    def split(self, n):
        ...
        EXAMPLES::
            ...
            Finite quadratic module in 9 generators:
             ...
             form: 1/23*x0^2 + 1/23*x1^2 + 1/23*x2^2 + 1/23*x3^2 + 1/4*x4^2 + 1/4*x5^2 + 1/4*x6^2 + 3/4*x7^2 + 2/3*x8^2
            sage: U = A.subgroup(A.gens()); U
             < d + e, c + f, b + g, a + h + 2*j >
            sage: U2 = U.split(2); U2
            (< e, f, g, h >, < 2*d, 2*c, 2*b, 2*a + j >)
            sage: U3 = U.split(3); U3
            (< 2*j >, < d + e, c + f, b + g, 3*a + h >)
            sage: U23 = U.split(23); U23
            (< 2*d, 2*c, 2*b, 6*a >, < e, f, g, h + j >)
        ...
        return FiniteQuadraticModule_subgroup(tuple(ul)), FiniteQuadraticModule_subgroup(tuple(vl))
...
def FiniteQuadraticModuleRandom(discbound=100,normbound=100,verbose=0):
    ...
    if verbose>0:
        print("A={0}".format(A))
        print("|A.list()|={0}".format(len(list(A))))
        print("N={0}".format(N))
    ...
    if False == B.is_nondegenerate():
        if verbose>0:
            ...
            print("A=",A)
            print("Gram(A)={0}".format(A.gram()))
            print("B={0}".format(B))
            print("N=A.kernel()={0}".format(N))
...
def orbitlist_test(str = None):
    ...
    for p in olist.keys():
        ...
        print("{0}: # of elements in the computed orbits sum up to the order of {1} : {2}, {3}, {4}".format(
                                                                    p.str(), st, order == orbitsum, order, orbitsum))
    return testpassed
