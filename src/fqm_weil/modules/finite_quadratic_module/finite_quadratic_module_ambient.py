...
from .jordan_decomposition import JordanDecomposition
...
class FiniteQuadraticModule_ambient(FiniteQuadraticModule_base):
    ...
    def isotropic_subgroups(self):
        ...
        for subgroup in self.subgroups():
            if set([self.Q(g) for g in subgroup.gens()]) == {0}:
...
def _FiniteQuadraticModule_from_string(S, **args):
    ...
    EXAMPLES::
        ...
        Finite quadratic module in 5 generators:
         gens: a, b, c, d, e
         form: 1/4*x0^2 + 1/4*x0*x1 + 1/4*x1^2 + 15/16*x2^2 + 1/4*x0*x3 + 1/2*x1*x3 + 5/16*x3^2...
        ...
        Finite quadratic module in 7 generators:
         gens: a, b, c, d, e, f, g
         form: 1/16*x0^2 + 1/16*x1^2 + 15/16*x2^2 + 1/4*x3^2 + 1/4*x3*x4 + 1/4*x4^2 + 1/3*x5^2 ...
